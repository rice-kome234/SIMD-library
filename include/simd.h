...
public:
	 ...
	 *   @details elementCountを省略すると、要素数0の出力用配列として作成できます。
	 */
	explicit Array(Engine &engine, std::size_t elementCount = {});

	/*!
	 *   @brief vectorの値から配列を作成
	 *   @param[in] engine 配列を紐づけるEngine
	 *   @param[in] values 初期値としてコピーするfloat配列
	 */
	Array(Engine &engine, const std::vector<float> &values);

	/*!
	 *   @brief 初期化リストの値から配列を作成
	 *   @param[in] engine 配列を紐づけるEngine
	 *   @param[in] values 初期値としてコピーするfloat値
	 */
	Array(Engine &engine, std::initializer_list<float> values);

	/*!
	 *   @brief サイズと初期値を指定して配列を作成
	 *   @param[in] engine 配列を紐づけるEngine
	 *   @param[in] elementCount 論理的なfloat要素数
	 *   @param[in] value 全要素へ設定する値
	 */
	Array(Engine &engine, std::size_t elementCount, float value);
	 ...
	 *   @param[in] expr 代入する式
	 *   @return 自分自身への参照
	 *   @details 実際の計算はEngine::execute()で行います。
	 *            代入先が空の場合は、式に含まれる配列と同じ要素数へ自動で変更します。
	 */
 ...
 *   @warning Engine::execute()の後、その前に作ったExpressionは使わないでください。
 */
class Expression
{
public:
...
private:
	...
	Engine *engine_{};
	std::size_t nodeId_{std::numeric_limits<std::size_t>::max()};
};
...
public:
	 ...
	 *   @note 新しいコードでは `Array{engine, elementCount}` の形でも作成できます。
	 */
	 ...
	 *   @param[in] values 初期値としてコピーするfloat配列
	 *   @return このエンジンに紐づいたArray
	 *   @note 新しいコードでは `Array{engine, values}` の形でも作成できます。
	 */
	 ...
	 *   @param[in] values 初期値としてコピーするfloat値
	 *   @return このエンジンに紐づいたArray
	 *   @note 新しいコードでは `Array{engine, {1.0f, 2.0f}}` の形でも作成できます。
	 */
	 ...
	 *   @param[in] value 全要素へ設定する値
	 *   @return このエンジンに紐づいたArray
	 *   @note 新しいコードでは `Array{engine, elementCount, value}` の形でも作成できます。
	 */
